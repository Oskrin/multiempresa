...
if (!$sidx)
    $sidx = 1;
$result = pg_query("SELECT COUNT(*) AS count from c_pagarexternas CP, proveedores P, usuario U where CP.id_proveedor = P.id_proveedor and CP.id_usuario = U.id_usuario and CP.id_empresa = '$_SESSION[id_empresa]'");
...
if ($start < 0)
    $start = 0;
if ($search == 'false') {
    $SQL = "select CP.id_c_pagarexternas, P.identificacion_pro, P.empresa_pro, CP.num_factura, CP.total, CP.fecha_actual from c_pagarexternas CP, proveedores P, usuario U where CP.id_proveedor = P.id_proveedor and CP.id_usuario = U.id_usuario  and CP.id_empresa = '$_SESSION[id_empresa]' ORDER BY $sidx $sord offset $start limit $limit";
} else {
