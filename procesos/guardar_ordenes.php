...
for ($i = 0; $i <= $nelem1; $i++) {
    ...
    pg_query("insert into ordenes_produccion values('$cont1','$_SESSION[id]','$_POST[comprobante]','$_POST[fecha_actual]','$_POST[hora_actual]','$arreglo1[$i]','$arreglo2[$i]','$_POST[subtotal]','Activo','$_SESSION[id_empresa]')");
    ////////////////////////////////////////
    //
    ...
    pg_query("Update productos Set precio_compra='" . $format_numero . "', iva_minorista = '" . $utilidad_numero_mi . "' , iva_mayorista = '" . $utilidad_numero_ma . "' , stock='" . $cal . "' where cod_productos='" . $arreglo1[$i] . "' and id_empresa = '$_SESSION[id_empresa]'");
    ///////////////////////////////////////////
}
...
for ($j = 0; $j <= $nelem2; $j++) {
    ...
    //
    //////////////modificar productos///////////
    $consulta2 = pg_query("select * from productos where cod_productos = '$arreglo3[$j]' and id_empresa = '$_SESSION[id_empresa]'");
    while ($row = pg_fetch_row($consulta2)) {
        $stock2 = $row[13];
    }
    $cal2 = $stock2 - $arreglo4[$j];

    pg_query("Update productos Set stock='" . $cal2 . "' where cod_productos='" . $arreglo3[$j] . "' and id_empresa = '$_SESSION[id_empresa]'");
    ///////////////////////////////////////////
}
