...
///////////////////////////////////////////
    for ($i = 1; $i < $nelem; $i++) {
        ...
        $cont1++;
        ////////////guardar pagos////////
        pg_query("insert into pagos_cobrar values('$cont1','$_POST[id_cliente]','$_SESSION[id]','$_POST[comprobante]','$_POST[fecha_actual]','$_POST[hora_actual]','$_POST[forma_pago]','$_POST[tipo_pago]','$arreglo2[$i]','$arreglo3[$i]','$arreglo4[$i]','$arreglo5[$i]','$arreglo6[$i]','$arreglo7[$i]','$_POST[observaciones]','Activo','$_SESSION[id_empresa]')");
        ////////////////////////////////////////
        //
        ////////////modificar pagos////////
        $consulta2 = pg_query("select * from c_cobrarexternas where id_c_cobrarexternas = '$arreglo1[$i]' and id_empresa = '$_SESSION[id_empresa]'");
        ...
        if ($format_numero == 0.00) {
            pg_query("Update c_cobrarexternas Set saldo='" . $format_numero . "', estado='Cancelado' where id_c_cobrarexternas='" . $arreglo1[$i] . "' and id_empresa = '$_SESSION[id_empresa]'");
        } else {
            pg_query("Update c_cobrarexternas Set saldo='" . $format_numero . "' where id_c_cobrarexternas='" . $arreglo1[$i] . "' and id_empresa = '$_SESSION[id_empresa]'");
            //////////////////////////////
        }
    }
    $data = 1;
    ...
    for ($i = 1; $i < $nelem; $i++) {
        ...
        //
        ////////modificar los pagos///
        $consulta2 = pg_query("select * from pagos_venta where id_pagos_venta = '$arreglo1[$i]' and id_empresa = '$_SESSION[id_empresa]'");
        while ($row = pg_fetch_row($consulta2)) {
            $saldo = $row[9];
        }
        ...
        if ($format_numero == 0) {
            pg_query("Update pagos_venta Set saldo='" . $format_numero . "', estado='Cancelado' where id_pagos_venta='" . $arreglo1[$i] . "' and id_empresa = '$_SESSION[id_empresa]'");
        } else {
            pg_query("Update pagos_venta Set saldo='" . $format_numero . "' where id_pagos_venta='" . $arreglo1[$i] . "' and id_empresa = '$_SESSION[id_empresa]'");
        }
        //////////////////////////////
        //
        ///////////modificar detalles pagos //////// 

        $consulta4 = pg_query("select * from detalles_pagos_internos order by id_detalles_pagos_interna asc");
