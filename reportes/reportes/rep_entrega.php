...
require('../dompdf/dompdf_config.inc.php');
	...
	<body>
      ...
      $consulta = pg_query("select * from registro_equipo,color,marcas,clientes,usuario,categoria where registro_equipo.id_color=color.id_color and registro_equipo.id_marca=marcas.id_marca and registro_equipo.id_cliente=clientes.id_cliente and registro_equipo.id_usuario=usuario.id_usuario and registro_equipo.id_categoria=categoria.id_categoria and registro_equipo.id_registro='$_GET[id]'");   
        ...
        $codigo.='</table>'; 
        $codigo.='<table id="tblEntrega6" style="margin-left:413px" >';     
        $codigo.='<tr>
            <td style="border:solid 1px;width:97px;height:24px;text-align:center">Subtotal: </td>';           
            $codigo.='<td style="border:solid 1px;width:97px;height:24px;text-align:center">';
            $codigo.="$ ".$sub;            
            $codigo.='</td>                       
        </tr></table>';
        $codigo.='<table id="tblEntrega7"  style="margin-left:413px">';     
        $codigo.='<tr>
            <td style="border:solid 1px;width:97px;height:24px;text-align:center">Descuento: </td>            
            <td style="border:solid 1px;width:97px;height:24px;text-align:center">';
            $codigo.="$ ".$des;
            $codigo.='</td>                        
        </tr></table> ';
        $codigo.='<table id="tblEntrega8" style="margin-left:413px;">';     
        $codigo.='<tr>
            <td style="border:solid 1px;width:97px;height:24px;text-align:center">Total a Pagar: </td>            
            <td style="border:solid 1px;width:97px;height:24px;text-align:center">';
            $codigo.="$ ".($sub-$des);
            $codigo.='</td>
        </tr></table>            
